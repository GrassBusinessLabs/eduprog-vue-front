      ...
      <VTable>
        ...
        <tbody>
          ...
          >
            ...
            >
              <draggable
                v-if="Object.keys(state.selected).length"
                :list="state.selected[item][semester]"
                :disabled="!enabled"
                item-key="name"
...
export default {
  ...
  setup(props, { emit }) {
    const state = reactive({
      editIndex: null,
      originalData: null,
      disciplines: [...new Set(props.scheme.map(e => e.discipline))],
      semesters: [...Array(8).keys()],
      changes: {
        subjects: [],
        semester: '',
    ...
    function mounted() {
      console.log('компоненты', components.value)
      this.initData()
    }
    function initData(){
      state.selected={}
      ...
      Object.keys(this.selected).map(key => {
        state.selected[key].forEach((semester, index) => {
          state.selected[key][index].push(...getComponentByDiscipline(key, index + 1))
        })
      })
    }
    ...
    function cancel(item) {
      ...
      if (!originalData.value) {
        itemsS.splice(itemsS.indexOf(item), 1)
    ...
    function getComponentByDiscipline(discipline, semestr) {
      let array =  scheme.value.filter(e => {
        if (e.discipline === discipline && e.semester_num === semestr) {
          return e.eduprogcomp.name
        }
      })
    ...
    function getSubjects() {
      return  components.value.mandatory.concat(components.value.selective)
    }

    function handleSubject(event, semester, discipline) {
      state.changes.subjects = event
      state.changes.semester = semester + 1
      state.changes.discipline = discipline
